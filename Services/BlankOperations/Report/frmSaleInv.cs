    ...
    public partial class frmSaleInv : Form
    {
        ...
        public frmSaleInv(IPosTransaction posTransaction, SqlConnection conn, bool bDuplicate, int iIsGift, int iLanForInv, int isItFromShowJour)
        {
            ...
            if (retailTrans != null)
            {
                ...
                if (string.IsNullOrEmpty(retailTrans.Customer.Name))
                {
                    ...
                    {
                        sCustName = Convert.ToString(retailTrans.PartnerData.LCCustomerName).trim();
                        sCustAddress = Convert.ToString(retailTrans.PartnerData.LCCustomerAddress).trim();
                ...
                if (Convert.ToString(ApplicationSettings.Terminal.StoreName) != string.Empty)
                    sStoreName = Convert.ToString(ApplicationSettings.Terminal.StoreName).Trim();
        ...
        private void frmSaleInv_Load(object sender, EventArgs e)
        {
            ...
            if (iInvLang == 1 && iGiftPrint == 0)
                ...
                GetTender(ref dsTender, "AR");
            else
                GetTender(ref dsTender, "Both");

            GetTaxDetail(ref dsTaxDetail); // added on 31/03/2014 RHossain

            GetCompanyLogo(ref dsCompanyLogo);// added on 26/08/2014 RHossain

            sAmtinwds = oBlank.Amtinwds(Math.Abs(dTotAmt + Convert.ToDouble(totTaxAmt))); // added on 28/04/2014 RHossain     

            decimal dAmt = decimal.Round((Math.Abs(Convert.ToDecimal(dTotAmt + Convert.ToDouble(totTaxAmt)))), 2, MidpointRounding.AwayFromZero);
            sAmtinwds = oBlank.Amtinwds(Convert.ToDouble(dAmt)); // added on 28/04/2014 RHossain               
            sAmtinwdsArabic = oBlank.AmtinwdsInArabic(Math.Abs(dTotAmt + Convert.ToDouble(totTaxAmt)));
            ...
            param[26] = new ReportParameter("totTaxableamt", Convert.ToString(totTaxableamt.ToString("#,0.00;(#,0.00)")));
            param[27] = new ReportParameter("totTaxAmt", Convert.ToString(totTaxAmt.ToString("#,0.00;(#,0.00)")));
            param[28] = new ReportParameter("CopyName", "Party Copy");
        ...
        private void GetSalesInvData(ref DataTable dtCol)
        {
            ...
            if (dtCol != null && dtCol.Rows.Count < 5 && dtCol.Rows.Count > 0)
            {
                for (int j = dtCol.Rows.Count; j < 5; j++)
                    dtCol.Rows.Add();
            }
        }
        ...
        private void GetTender(ref DataSet dsSTender, string sLang = "")
        {
            string sStoreNo = ApplicationSettings.Terminal.StoreId;
            string sqlsubDtl = "";
            if (string.IsNullOrEmpty(sLang))
            {
                sqlsubDtl = " DECLARE @CHANNEL bigint  SELECT @CHANNEL = ISNULL(RECID,0) FROM RETAILSTORETABLE WHERE STORENUMBER = '" + sStoreNo + "'" +
            ...
            else if (sLang == "AR")
            {
                ...
                sqlsubDtl = " DECLARE @CHANNEL bigint  SELECT @CHANNEL = ISNULL(RECID,0) FROM RETAILSTORETABLE WHERE STORENUMBER = '" + sStoreNo + "'" +
                                    "  SELECT N.ARABICNAME + ' (' + M.CURRENCY + ')' as NAME, M.TENDERTYPE,CAST(ISNULL(M.AMOUNTCUR,0) AS DECIMAL(28,2)) AS AMOUNT," +
                                    " (ISNULL(CARDORACCOUNT,'') + ISNULL(CREDITVOUCHERID,'') + ISNULL(stuff(ISNULL(GIFTCARDID,''),1,LEN(ISNULL(GIFTCARDID,''))-4,REPLICATE('x', LEN(ISNULL(GIFTCARDID,''))-4)),'')) AS CARDNO" + //+ ISNULL(GIFTCARDID,'') ADDED ON 16/07/2014
                                    " FROM RETAILTRANSACTIONPAYMENTTRANS M" +
                                    " LEFT JOIN RETAILSTORETENDERTYPETABLE N ON M.TENDERTYPE = N.TENDERTYPEID" +
                                    " WHERE M.TRANSACTIONID = @TRANSACTIONID AND N.CHANNEL = @CHANNEL  AND M.TERMINAL = '" + sTerminal + "' AND M.TRANSACTIONSTATUS = 0 " +
                    //" UNION SELECT F.NAME AS NAME,100 AS TENDERTYPE, (ABS(CAST(ISNULL(A.TAXAMOUNT,0)AS DECIMAL(18,2)))" +
                                    " UNION ALL SELECT F.DESCRIPTION AS NAME,100 AS TENDERTYPE, (ABS(CAST(ISNULL(A.TAXAMOUNT,0)AS DECIMAL(18,2)))" +
                                    " + ABS(CAST(ISNULL(A.NETAMOUNT,0)AS DECIMAL(28,2))))AS AMOUNT," +
                                    " + ' TransactionId : ' + b.ADVANCEADJUSTMENTID " +
                                    " + ' Receipt : ' + (select RECEIPTID from RETAILTRANSACTIONPAYMENTTRANS where TRANSACTIONID=b.ADVANCEADJUSTMENTID) " +
                                    " CARDNO" +
            ...
            else
            {
                ...
                sqlsubDtl = " DECLARE @CHANNEL bigint  SELECT @CHANNEL = ISNULL(RECID,0) FROM RETAILSTORETABLE WHERE STORENUMBER = '" + sStoreNo + "'" +
                                    "  SELECT N.NAME  + ' (' + M.CURRENCY + ')' + ' '  +  N.ARABICNAME as NAME, M.TENDERTYPE,CAST(ISNULL(M.AMOUNTCUR,0) AS DECIMAL(28,2)) AS AMOUNT," +
                                    " (ISNULL(CARDORACCOUNT,'') + ISNULL(CREDITVOUCHERID,'') + ISNULL(stuff(ISNULL(GIFTCARDID,''),1,LEN(ISNULL(GIFTCARDID,''))-4,REPLICATE('x', LEN(ISNULL(GIFTCARDID,''))-4)),'')) AS CARDNO" + //+ ISNULL(GIFTCARDID,'') ADDED ON 16/07/2014
                                    " FROM RETAILTRANSACTIONPAYMENTTRANS M" +
                                    " LEFT JOIN RETAILSTORETENDERTYPETABLE N ON M.TENDERTYPE = N.TENDERTYPEID" +
                                    " WHERE M.TRANSACTIONID = @TRANSACTIONID AND N.CHANNEL = @CHANNEL  AND M.TERMINAL = '" + sTerminal + "' AND M.TRANSACTIONSTATUS = 0 " +
                    //" UNION SELECT F.NAME AS NAME,100 AS TENDERTYPE, (ABS(CAST(ISNULL(A.TAXAMOUNT,0)AS DECIMAL(18,2)))" +
                                    " UNION ALL SELECT F.NAME + ' '  +  F.DESCRIPTION  AS NAME,100 AS TENDERTYPE, (ABS(CAST(ISNULL(A.TAXAMOUNT,0)AS DECIMAL(18,2)))" +
                                    " + ABS(CAST(ISNULL(A.NETAMOUNT,0)AS DECIMAL(28,2))))AS AMOUNT," +
                                    " + ' TransactionId : ' + b.ADVANCEADJUSTMENTID " +
                                    " + ' Receipt : ' + (select RECEIPTID from RETAILTRANSACTIONPAYMENTTRANS where TRANSACTIONID=b.ADVANCEADJUSTMENTID) " +
                                    " CARDNO" +
