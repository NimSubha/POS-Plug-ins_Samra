...
/// <summary>
/// Summary description for MultiCurrency 
/// added on 02/09/2014 RHossain for multi currency purpose
/// </summary>
namespace Microsoft.Dynamics.Retail.Pos.BlankOperations
{
    ...
    public class MultiCurrency
    {
        ...
        public string ConvertToWord(string Value)
        {
            ...
            if (!(Value.ToString().Length > 40))
            {
                if(!IsNumeric(Value.ToString()))
                {
                    ...
                    if(sI2.ToString().Length == 1)
                        sAfterDecimalValue = sI2.ToString() + "" + "0";
                    else
                        sAfterDecimalValue = sI2.ToString();

                    sFills = " And " + GetWordConversion(sAfterDecimalValue) + GetCurrencySuffixWithCode();//" Fills";
                }
                else
                {
                ...
                if(IsNumeric(iValue.ToString()))
                {
                    try
                    {
                        string strValue = Reverse(Convert.ToString(iValue));
                        switch (strValue.Length)
                        {
                            case 1:
                                if (int.Parse(strValue.ToString()) > 0)
                                    ConvertedString = GetWordConversion(strValue);//Value
                                else
            ...
            {
                ConvertedString = "Please Enter Value in Less Than or Equal to 40 Digit";
            }
            return ConvertedString + sFills;
        }
            ...
            if (inputNumber.Length <= 3 && inputNumber.Length > 0)
            {
                if (inputNumber.Length == 3)
                {
                    if (int.Parse(inputNumber.Substring(0, 1)) > 0)
                        ToReturnWord = ToReturnWord + StaticSuffixFind(inputNumber.Substring(0, 1)) + "Hundred ";
        ...
        private string GetCurrencySuffixWithCode() 
        {
            SqlConnection conn = new SqlConnection();
            conn = ApplicationSettings.Database.LocalConnection;

            StringBuilder commandText = new StringBuilder();
            commandText.Append("SELECT CRWSuffix FROM CURRENCY WHERE CURRENCYCODE='" + ApplicationSettings.Terminal.StoreCurrency + "'");
